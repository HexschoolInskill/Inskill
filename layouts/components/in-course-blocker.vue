<template>
    <div class="absolute gradient left-[50%] translate-x-[-50%] p-4 flex z-10 text-white w-full h-[70vh] top-[15vh] rounded-lg">
        <div class="w-8/12">
            <img :src="props.currentCourse.thumbnail" alt="thumbnail">

            <article class="my-4">
                <h1 class="font-bold text-3xl">直播內容介紹</h1>
                <p v-html="props.currentCourse.description"></p>
            </article>
        </div>
        <div class="w-4/12">
            <div class="mb-4">
                <h1 class="font-bold text-3xl">{{ props.currentCourse.title }}</h1>
                <p>開課老師名稱</p>                
            </div>
            <div class="mb-4">
                <h2 class="font-bold text-3xl">直播時間</h2>
                <p>{{ props.currentCourse.startTime }}</p>                
            </div>
            <div class="mb-4">
                <h2 class="font-bold text-3xl">參加人數</h2>
                <p>{{ props.currentCourse.purchasedCount }} 人已報名</p>                
            </div>
            <div class="mb-4">
                <h2 class="font-bold text-3xl">直播售價</h2>
                <p>NT${{ props.currentCourse.price }}</p>                
            </div>

            <button class="flex fixed bottom-[40px] bg-black text-white rounded px-4 py-1" type="button">
                <svg
                class="w-[25px]"
                version="1.1"
