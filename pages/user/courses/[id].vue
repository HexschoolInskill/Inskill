<template>
    <in-container>
        ...
        <div class="flex border-bottom text-white pt-8 pb-4 items-center mb-4">
            <h1 class="font-bold text-3xl mr-auto">
                課程名稱
            </h1>

            <h2 class="font-bold text-2xl mr-4">NT$ {{ currentCourse.price }}</h2>

            <button class="mx-2 border border-white rounded bg-white text-black px-3 py-2" type="button">加入購物車</button>
            <button class="mx-2 border border-white rounded bg-white text-black px-3 py-2" type="button">加入收藏</button>
        </div>

        <div class="flex text-white">

            <aside class="w-2/12">
                <h2 class="font-bold text-2xl my-4">課程介紹</h2>

                <ul>
                    <li 
                    v-for="(chapter, index) in currentCourse.chapters"
                    :key="chapter.title"
                    class="border rounded-md px-4 my-2">
            ...
            <main class="p-4 w-10/12">
                <div class="flex">
                    <div class="rounded-lg wrapper p-4 w-11/12" style="background: linear-gradient(90deg,rgb(38, 43, 47),#000)">
                        <div class="flex mb-10">
                            <h1 class="font-bold text-3xl mr-auto">{{ currenChapterLession.title }}</h1>
                            <span v-if="currenChapterLession.freePreview" class="border rounded px-6">試看</span>
                        </div>

                        <div v-for="(content, index) in currenChapterLession.lessionContent" :key="index">
                        
                            <in-content-video v-if="content.contentType === 'video'" :source="content.content"></in-content-video>
                            
                            <in-content-text v-if="content.contentType === 'text'" :source="content.content"></in-content-text>
