...
export class WebRTCAdaptor {
  ...
  sendPublishCommand(
    ...
    var jsCmd = {
      ...
      subscriberCode:
        typeof subscriberCode !== undefined && subscriberCode != null ? subscriberCode : '',
  ...
  play(streamId, token, roomId, enableTracks, subscriberId, subscriberCode, metaData) {
    ...
    var jsCmd = {
      ...
      subscriberId: typeof subscriberId !== undefined && subscriberId != null ? subscriberId : '',
      subscriberCode:
        typeof subscriberCode !== undefined && subscriberId != null ? subscriberCode : '',
  ...
  tryAgain() {
    ...
    if (
      this.remotePeerConnection[this.publishStreamId] != null &&
      ...
      this.publish(
        ...
        this.publishToken,
        this.publishSubscriberId,
        this.publishSubscriberCode,
        this.publishStreamName,
        this.publishMainTrack,
  ...
  iceCandidateReceived(event, streamId) {
    if (event.candidate) {
      ...
      if (protocolSupported) {
        ...
        if (event.candidate.candidate != '') {
          //
          this.notifyErrorEventListeners(
  ...
  initPeerConnection(streamId, dataChannelMode) {
    ...
    if (this.remotePeerConnection[streamId] == null) {
      ...
      console.log(
        'stream id in init peer connection: ' + streamId + ' close stream id: ' + closedStreamId
      )
      ...
      if (!this.playStreamId.includes(streamId)) {
        if (this.mediaManager.localStream != null) {
          ...
          this.mediaManager.localStream
            .getTracks()
            .forEach((track) =>
              this.remotePeerConnection[streamId].addTrack(track, this.mediaManager.localStream)
            )
        }
      }
  ...
  addIceCandidate(streamId, candidate) {
    ...
    if (protocolSupported) {
      this.remotePeerConnection[streamId]
        ...
        .catch((error) => {
          console.error(
            'ice candiate cannot be added for stream id: ' + streamId + ' error is: ' + error
          )
  ...
  enableAudioLevel(stream, streamId) {
    ...
    soundMeter.connectToSource(stream, null, function (e) {
      ...
      console.log(
        'Added sound meter for stream: ' + streamId + ' = ' + soundMeter.instant.toFixed(2)
      )
    })
