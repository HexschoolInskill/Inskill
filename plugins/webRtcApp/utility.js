...
export function errorHandler(error, message) {
  ...
  if (error.indexOf('WebSocketNotConnected') != -1) {
    ...
    errorMessage =
      "Data store is not available. It's likely that server is initialized or getting closed"
  } else {
    if (error.indexOf('NotFoundError') != -1) {
      errorMessage = 'Camera or Mic are not found or not allowed in your device'
    } else if (error.indexOf('NotReadableError') != -1 || error.indexOf('TrackStartError') != -1) {
      errorMessage =
        'Camera or Mic are already in use and they cannot be opened. Choose another video/audio source if you have on the page below '
    } else if (
      ...
      errorMessage =
        'Please Install SSL(https). Camera and mic cannot be opened because of unsecure context. '
