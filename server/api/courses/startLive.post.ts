...
// import {getWebRTCAppList , createWebRTCApp, updateWebRTCAppEndPointList} from '../../services/mediaServer'
...
//         const {success : webRTCAppListResponseSuccess, data : webRTCAppListResponse} = await getWebRTCAppList()
...
//             const {success : createWebRTCAppResponseSuccess, data : createWebRTCAppResponse} = await createWebRTCApp(`${Date.now()}`)
...
//                 })
//             }
//             const { data : createWebRTCAppData } = createWebRTCAppResponse
//             // get streamId from data
//             const { streamId } = createWebRTCAppData
//             // update webRTCApp endPointList
//             const {success : updateWebRTCAppEndPointListResponseSuccess, data : updateWebRTCAppEndPointListResponse} = await updateWebRTCAppEndPointList(streamId, `${youtubeRTMPEndpoint}/${liveResponse.id}`)
...
//         } else {
//             // update webRTCApp endPointList
//             const {success : updateWebRTCAppEndPointListResponseSuccess, data : updateWebRTCAppEndPointListResponse} = await updateWebRTCAppEndPointList(webRTCApp.streamId, `${youtubeRTMPEndpoint}/${liveResponse.id}`)
//             if (!updateWebRTCAppEndPointListResponseSuccess) {
//                 return createError({
//                     statusCode: 400,
//                     message: 'Update webRTCApp endPointList fail'
