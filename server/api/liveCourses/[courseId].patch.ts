...
import models, { LiveCourse } from '../../model/schema'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const course = (await models.LiveCourse.findById(courseId)) as LiveCourse // 添加類型斷言為 LiveCourse
    if (!course) {
      return createError({
        statusCode: 400,
    ...
    if (contenType === 'application/json') {
      ...
      // update course
      const { title, description, price, startTime, endTime, isPublic } = value
      course.title = title
      ...
      const blob = bucket.file(
        `images/liveCourse/${courseId}.${file.originalname.split('.').pop()}`
      )
