...
let ACCESS_TOKEN =
  'ya29.a0AWY7CklY3isjoB22FKx0Z4NI1RgMzgAnYBssuuv-5M7ETEmFF9kpzSfemUvbesqHNYzXsA70FJ02_BpztrugdCagosM-m1zkFQ6_3lM32UgwQ-whwQVfGsdN3DJ2mVa4HYg6TOQWJTeEBW2QgqOjFBtH0AzxGvIaCgYKAe8SARASFQG1tDrp-rVpXdwAX56xh1hMaZdFYQ0166'
...
async function createLiveBroadcast(
  title: string = `Live Stream ${Date.now()}`,
  description: string = `Live Stream ${Date.now()}`,
...
async function bindBroadcast2Stream(liveBroadcastId: string, liveStreamId: string) {
  try {
    const bindResponse = await fetch(
      `https://www.googleapis.com/youtube/v3/liveBroadcasts/bind?id=${liveBroadcastId}&streamId=${liveStreamId}&part=id,contentDetails`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${ACCESS_TOKEN}`,
...
async function updateBroadcastStatus(broadcastId: string, broadcastStatus: string) {
  try {
    const response = await fetch(
      `https://www.googleapis.com/youtube/v3/liveBroadcasts/transition?part=status&id=${broadcastId}&broadcastStatus=${broadcastStatus}`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${ACCESS_TOKEN}`,
